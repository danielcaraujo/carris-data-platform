...
class MergeGTFSAndEndpointTablesTask:
    ...
    def _create_spark_session(self):
        ...
        return (
            SparkSession.builder.appName("MergeGTFSAndEndpointTablesTask")
            .config(conf=conf)
            .getOrCreate()
        )

    def write_to_bigquery(
        self, df: DataFrame, table_name: str, write_mode: str = "overwrite"
    ):
        """
    ...
    def process_table(
        ...
        gtfs_df = (
            ...
            .option(
                "table",
                f"{self.project_id}.{self.dataset_id}.staging_gtfs_{table_name}",
            )
            .load()
        )
        endpoint_df = (
            ...
            .option(
                "table",
                f"{self.project_id}.{self.dataset_id}.staging_endpoint_{table_name}",
            )
            .load()
        )

        gtfs_df_new_cols = gtfs_df.select(
            [col for col in gtfs_df.columns if col not in endpoint_df.columns]
        )
        merged_df = endpoint_df.join(
            gtfs_df_new_cols,
            endpoint_df.id == gtfs_df_new_cols[f"{table_name[:-1]}_id"],
            "inner",
        )
        ...
        if columns_to_rename:
            for rename in columns_to_rename:
                merged_df = merged_df.withColumnRenamed(
    ...
    def run(self, tables, write_mode="overwrite"):
        ...
        try:
            for table in tables:
                self.process_table(
                    table["name"],
                    table["columns_to_exclude"],
                    table["columns_to_rename"],
                    write_mode,
                )
...
tables = [
    ...
    },
    {
        ...
        "columns_to_rename": [
            {"old_name": "stop_lat", "new_name": "stop_latitude"},
            {"old_name": "stop_lon", "new_name": "stop_longitude"},
        ],
    },
]
