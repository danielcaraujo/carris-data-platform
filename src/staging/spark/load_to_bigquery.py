...
def clean_dataframe(df):
    ...
    for name, dtype in df.dtypes:
        if dtype in ["float", "double"]:
            df = df.withColumn(
                name,
                F.when(F.isnan(F.col(name)) | F.col(name).isNull(), F.lit(0)).otherwise(
                    F.col(name)
                ),
            )
    ...
    for name, dtype in df.dtypes:
        if dtype == "string":
            df = df.withColumn(
                name,
                F.when(F.col(name).isNull(), F.lit(
                    "UNKNOWN")).otherwise(F.col(name)),
            )
...
class BucketToBigQueryTask:
    ...
    def _create_spark_session(self):
        ...
        return (
            SparkSession.builder.appName("BucketToBigQueryTask")
            .config(conf=conf)
            .getOrCreate()
        )
    ...
    def write_to_bigquery(
        ...
        df = df.withColumn(
            "partition_date", F.to_date(F.col("partition_date"), "yyyyMMdd")
        )
    ...
    def process_table(self, table_name, table_sources, write_mode="overwrite"):
        ...
        source_dfs = list(
            map(
                lambda source: self.process_source(
                    table_name,
                    source,
                    schemas[
                        (
        ...
        if len(source_dfs) == 1:
            ...
            for df, table_source in zip(source_dfs, table_sources):
                self.write_to_bigquery(
                    df, f"{table_source}_{table_name}", write_mode)
