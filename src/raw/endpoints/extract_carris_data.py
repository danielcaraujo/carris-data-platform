...
def extract_endpoint_to_spark(spark, endpoint, gcs_dir):
    ...
    output_path = f"{gcs_dir}/extracted_at={PARTITION_VALUE}/{entity_name}"
            ...
            for filename in zip_file.namelist():
                if filename.endswith('.txt'):
                    ...
                    with zip_file.open(filename) as f:
                        ...
                        spark_df = pandas_to_spark(spark, df)
                        spark_df = spark_df.withColumn("partition_date", lit(PARTITION_VALUE))
                        output_path = f"{gcs_dir}/gtfs/extracted_at={PARTITION_VALUE}/{table_name}"
                        spark_df.write.mode("overwrite").partitionBy("partition_date").parquet(output_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--endpoint", type=str, default=None,
                        help="Single API endpoint to extract (e.g. /stops, gtfs, /facilities/stores). If omitted, extracts all endpoints and GTFS.")
    ...
    if args.endpoint:
        ...
        endpoints = [
            ...
            "/service_alerts", "/realtime_arrivals"
        ]
